...
DEFAULT_DATASET_SPLIT_FILE = 'dataset_split.csv'
DEFAULT_HISTOGRAM_REF_FILE = 'histogram_ref_file.txt'
...
def add_application_args(parser):
    ...
    parser.add_argument(
        "--job-dir",
        metavar='',
        help="Directory to save/load intermediate training models",
        required=True)
...
def add_network_args(parser):
    ...
    parser.add_argument(
        "--uniform_sampler_positive_fraction",
        help="Fraction of samples with positive pixels."
             "This setting gives the lower limit, the actual value can larger.",
        type=float,
        default=0.0)
...
def add_training_args(parser):
    ...
    parser.add_argument(
        "--lr_stop_value",
        help="Stop training when learning rate reaches the given value.",
        type=float,
        default=0.0)

    parser.add_argument(
        "--lr_decay_window",
        help="Iteration window for learning rate decay."
             "If loss is not reduced within the window, LR is reduced."
    ...
    parser.add_argument(
        "--dice_epsilon",
        metavar='',
        help='Regularization term for Dice-like losses',
        type=float,
        default=1.0)
