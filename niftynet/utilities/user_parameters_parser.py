...
def run(app_name=None):
    ...
    config_file_name = __resolve_config_file_path(meta_args.config)
    config = NiftyNetLaunchConfig()
    with tf.io.gfile.GFile(config_file_name, 'r') as f:
        config.readfp(f)
    ...
    system_args['SYSTEM'].action = \
        look_up_operations(meta_args.task, ACTIONS)
    if system_args['SYSTEM'].log_dir is None:
        system_args['SYSTEM'].log_dir = os.path.join(system_args['SYSTEM'].job_dir, 'logs')
