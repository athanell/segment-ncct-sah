...
class ApplicationDriver(object):
    ...
    def initialise_application(self, workflow_param, data_param=None):
        ...
        self.log_dir = system_param.log_dir
        ...
        if data_param:
            do_new_partition = not tf.io.gfile.exists(system_param.dataset_split_file)
    ...
    def run(self, application, graph=None):
        ...
        with tf.compat.v1.Session(config=tf_config(), graph=graph):
            try:
    ...
    def loop(application,
        ...
        ``tf.compat.v1.Session`` or ``session.as_default()``.

        :param application: a niftynet.application instance, application
            will provides ``tensors`` to be fetched by ``tf.compat.v1.Session.run()``.
    ...
    def loop_step(application, iteration_message):
        """
        Calling ``tf.compat.v1.Session.run`` with parameters encapsulated in
