...
by ``tf.compat.v1.get_default_session()`` and ``tf.compat.v1.get_default_graph()``.
"""
...
ITER_STARTED = NIFTYNET.signal(
    'iteration_started',
    doc='emitted when every iteration starts, before ``tf.compat.v1.Session.run(...)``.')

#: Signal emitted before the end of each iteration
#: (after the creation of both graph and session.)
ITER_FINISHED = NIFTYNET.signal(
    'iteration_finished',
    doc='emitted at the end of each iteration, after ``tf.compat.v1.Session.run(...)``.')
