...
"""
from niftynet.engine.application_variables import NETWORK_OUTPUT, CONSOLE
...
class OutputInterpreter(object):
    ...
    def interpret_output(self, sender, **msg):
        ...
        """
        _iter_msg = msg['iter_msg']
        lr_stop_reached = False
        if _iter_msg.is_training:
            lr_stop_reached = sender.adjust_learning_rate(_iter_msg.current_iter_output[CONSOLE]['loss'])
        waiting_for_more_output = sender.interpret_output(
            _iter_msg.current_iter_output[NETWORK_OUTPUT])
        if lr_stop_reached or not waiting_for_more_output:
