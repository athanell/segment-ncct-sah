...
@register_image_loader('nifti', requires='nibabel')
def imread_nifti(filename):
    """Default nifti1 loader for NiftyNet."""
    try:
        with tf.io.gfile.GFile(filename, 'rb') as f_raw:
            with gzip.GzipFile(fileobj=f_raw, mode='rb') as f:
                fmap = {'image': nib.FileHolder(fileobj=f)}
                img = nib.Nifti1Image.from_file_map(fmap)
                return img, img.get_fdata()
    except:
        raise IOError('Nibabel could not load nifti file: {}'.format(filename))
