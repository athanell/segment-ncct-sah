...
from niftynet.io.image_loader import load_image
...
class SpatialImage2D(DataFromFile):
    ...
    def shape(self):
        ...
        if self._original_shape is None:
            try:
                self._original_shape = tuple(image_obj.header['dim'][1:6] for image_obj in self._image_obj)
    ...
    def _load_image(self):
        ...
        for file_i, loader_i in zip(self.file_path, self.loader):
            image_obj, fdata = load_image(file_i, loader_i)
            self._image_obj.append(image_obj)
            self._fdata.append(fdata)
    ...
    def dtype(self):
        ...
        """
        if not self._dtype:
            self._dtype = tuple(image_obj.header.get_data_dtype() for image_obj in self._image_obj)
            self._dtype = tuple(dtype_casting(dtype, interp_order) for dtype, interp_order in zip(self._dtype, self.interp_order))
        return self._dtype
    ...
    @classmethod
    def _load_single_file(cls, fdata, dtype=np.float32):
        image_data = misc.expand_to_5d(fdata)
    ...
    def get_data(self):
        if self._image_data is None:
            if len(self._file_path) > 1:
                image_data = []
                for fdata, dtype in zip(self._fdata, self._dtype):
                    data_array = self._load_single_file(fdata, dtype)
                    image_data.append(data_array)
                try:
                    self._image_data = np.concatenate(image_data, axis=4)
                except ValueError:
                    tf.compat.v1.logging.fatal(
                ...
                self._image_data = self._load_single_file(self._fdata[0], self.dtype[0])
            self._fdata = None
        return np.copy(self._image_data)
...
class SpatialImage3D(SpatialImage2D):
    ...
    def _load_single_file(self, fdata, dtype=np.float32):
        image_data = SpatialImage2D._load_single_file(fdata, dtype)
    ...
    def get_data(self):
        if self._image_data is None:
            if len(self.file_path) == 1:
                ...
                self._image_data = SpatialImage3D._load_single_file(self, self._fdata[0])
            else:
                mod_list = []
    ...
    def create_instance(cls, file_path, **kwargs):
        ...
        try:
            ...
            if os.path.isfile(file_path):
                ...
                ndims = 3 # misc.infer_ndims_from_file(file_path, loader)
        ...
        if image_type is None:
            try:
                ...
                loader = kwargs.get('loader', None) or (None,)
                ndims = 3 # misc.infer_ndims_from_file(file_path[0], loader[0])
