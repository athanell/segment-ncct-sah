...
class ImageReader(Layer):
    ...
    def layer_op(self, idx=None, shuffle=True):
        ...
        if False:
            ...
            import os
            for name, data in image_data_dict.items():
                file_path = './dump_' + os.path.basename(image_dict[name].file_path[0])
                print('Dumping preprocessed {} data to {}'.format(name, file_path))
                nib.Nifti1Image(np.squeeze(data, axis=(3,4)), np.eye(4)).to_filename(file_path)
