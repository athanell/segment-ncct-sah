...
def resolve_job_dir(dir_str, create_new=False):
    """
    Interpret `dir_str` as an absolute folder path.
    ...
    try:
        ...
        if os.path.isdir(dir_str):
            return os.path.abspath(dir_str)
    except TypeError:
        pass
    ...
    try:
        ...
        if os.path.isfile(dir_str):
            return os.path.abspath(os.path.dirname(dir_str))
    except TypeError:
        pass

    if create_new:
        # try to create the folder
        folder_path = touch_folder(dir_str)
        return folder_path
    else:
        raise ValueError("Could not resolve [{}]".format(dir_str))


def to_absolute_path(input_path, root):
    """
...
def get_latest_subfolder(parent_folder, create_new=False):
    ...
    try:
        log_sub_dirs = tf.io.gfile.listdir(parent_folder)
    except OSError:
    ...
    if log_sub_dirs and create_new:
        ...
        latest_valid_id = max(
            [int(name) for name in log_sub_dirs
             if tf.io.gfile.isdir(os.path.join(parent_folder, name))])
...
def set_logger(file_name=None):
    ...
    if file_name and not file_name.startswith('gs:'):
        # adding file output
...
def copyfile_if_possible(src, dst):
    if tf.io.gfile.exists(src) and src != dst:
        with tf.io.gfile.GFile(src, 'r') as f_src:
            with tf.io.gfile.GFile(dst, 'w') as f_dst:
