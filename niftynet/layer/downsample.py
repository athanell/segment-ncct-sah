    ...
    def layer_op(self, input_tensor):
        ...
        if self.func == 'CONSTANT':
            ...
            if isinstance(self.dims, int):
                window_shape = kernel_size_all_dims[:self.dims] + (1,)*len(kernel_size_all_dims[self.dims:])
                stride_all_dims = stride_all_dims[:self.dims] + (1,)*len(stride_all_dims[self.dims:])
            else:
